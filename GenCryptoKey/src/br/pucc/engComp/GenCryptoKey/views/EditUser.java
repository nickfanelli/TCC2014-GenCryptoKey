...
//    				String autoGeneratedBackupPassword = pwManager.generateBackupPassword();
    				...
    				// Calculate SHA-256 hash of the user's input and backup passwords before storing in database
//					StringBuffer userPasswordHash = new StringBuffer();
//					StringBuffer userBackupPasswordHash = new StringBuffer();
//					try {
//						MessageDigest mDigest = MessageDigest.getInstance("SHA-256");
//						byte[] userPwByteArray = mDigest.digest(valueFields.get(5).getText().getBytes());
//						for (int i = 0; i < userPwByteArray.length; i++) {
//							userPasswordHash.append(Integer.toString((userPwByteArray[i] & 0xff) + 0x100, 16).substring(1));
//						}
//						byte[] userBkpPwByteArray = mDigest.digest(autoGeneratedBackupPassword.getBytes());
//						for (int i = 0; i < userBkpPwByteArray.length; i++) {
//							userBackupPasswordHash.append(Integer.toString((userBkpPwByteArray[i] & 0xff) + 0x100, 16).substring(1));
//						}
//					}catch(NoSuchAlgorithmException nsae) {
//						nsae.printStackTrace();
//					}
//					
//    				newUser.setPassword(userPasswordHash.toString());
//    				newUser.setBackupPassword(userBackupPasswordHash.toString());
    				
    				editUser.setPassword(userInputPassword);
//    				editUser.setBackupPassword(autoGeneratedBackupPassword);
    				
//    				System.out.println("Backup password: " + autoGeneratedBackupPassword);
    				
    				try{
    					if(UserDAO.getInstance().updateUser(editUser, false) != -1) {
    	...
    	deleteUserButton.addActionListener(new ActionListener(){
    		...
    		public void actionPerformed(ActionEvent evt) {
				...
				if(confirmDeletion == JOptionPane.YES_OPTION) {
					try{
    					if(UserDAO.getInstance().deleteUser(editUser) != -1) {
    						editUserFrame.dispose();
	...
	public void focusLost(FocusEvent e) {
		...
		switch(textField.getName()){
			...
			else textField.setBackground(new Color(0, 200, 40));
			break;
			case "email":
